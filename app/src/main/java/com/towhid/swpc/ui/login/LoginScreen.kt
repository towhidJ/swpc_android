...
import com.towhid.swpc.viewmodel.AuthState
import com.towhid.swpc.viewmodel.AuthViewModel

@Composable
fun  LoginScreen(paddingValues: PaddingValues,viewModel: AuthViewModel= viewModel()){
    ...
    LaunchedEffect(authState) {
        when (authState) {
            ...
            is AuthState.Success -> {
                isLoading = false
                Toast.makeText(context, "Welcome ${(authState as AuthState.Success).response.user.userName}", Toast.LENGTH_LONG).show()
            }
            is AuthState.Error -> {
                isLoading = false
    ...
    Column (modifier = Modifier
        ...
        Button(
            onClick = {
                ...
                if(emailError.isNotEmpty() && passwordError.isEmpty())
                {

                }
            },
            modifier = Modifier.fillMaxWidth().padding(90.dp),
            enabled = !isLoading
        ) {
            if (isLoading) {
